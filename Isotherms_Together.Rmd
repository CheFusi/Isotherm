...
new_column_names<-c("group","H2O2_conc","Sample_ID",
                    ...
                    "Dose",
                    "Q_NH4","Q_Na","Q_K","Q_Mg", "Q_Cl","Q_PO4","Q_SO4","Q_Ace",
                    "Na_Conc_sd", "NH4_Conc_sd","K_Conc_sd","Mg_Conc_sd",
                    "Ace_Conc_sd","Cl_Conc_sd","PO4_Conc_sd","SO4_Conc_sd","Carb_Conc_sd",
                    ...
                    "Q_Ace_sd","Q_NH4_sd_perc","Q_Na_sd_perc","Q_K_sd_perc", "Q_Mg_sd_perc",
...
colnames(int_ALL)[colnames(int_ALL) == "Ox"] <- "H2O2_conc"
...
int_ALL$H2O2_conc <- c(05,10, 15, 20, 30, 30, 5, 10, 15, 20, 25, 30, 10, 12, 15, 20, 00)
...
ALL_Biochars$Q_NH4_SSA <- ALL_Biochars$Q_NH4/ALL_Biochars$SSA

ALL_Biochars$Q_NH4_SSA_sd <- ALL_Biochars$Q_NH4_sd/ALL_Biochars$SSA

#Normalizing to OC
ALL_Biochars$Q_NH4_OC <- ALL_Biochars$Q_NH4/ALL_Biochars$OC


#updating 0 to 00 and 5 to 05
ALL_Biochars <- ALL_Biochars %>%
  mutate(H2O2_conc = as.numeric(as.character(H2O2_conc)),
         H2O2_conc = case_when(
           H2O2_conc < 10 ~ sprintf("%02d", H2O2_conc),
           TRUE ~ as.character(H2O2_conc)
         ))
ALL_Biochars$H2O2_conc<- as.factor(ALL_Biochars$H2O2_conc)

#creating Type_ox column that has type (AK) and ox conc (05)
ALL_Biochars <-transform(ALL_Biochars, Type_ox = paste0(Type, "_", H2O2_conc))

# Loop through each column and replace NaN with NA
#ALL_Biochars <- apply(ALL_Biochars, 2, function(x) ifelse(is.nan(x), NA, x))
```

## Q Corrected for losses to headspace using control
...
ALL_Biochars$Q_Corr<-Q_function(ALL_Biochars,"Q_Corr","NH4_In","NH4_Conc","NH4_ctrl","volume","mass_biochar")

#removal efficiency
ALL_Biochars$Rem_Eff <- ((ALL_Biochars$NH4_In - ALL_Biochars$NH4_Conc)/ALL_Biochars$NH4_In)*100
...
Q_Conc_Plot<- ggplot(ALL_Biochars,aes(x=NH4_Conc,y=Q_NH4, color = OC,border= OC, fill=OC, shape=Dose, na.rm=TRUE)) +
...
Q_Act_Plot_3<- ggplot(ALL_Biochars,aes(x=NH4_Act,y=Q_NH4, color = OC,border= OC, fill=OC, na.rm=TRUE)) +
   geom_point(shape = 21, size = 6, stroke = 2, color = "black") +
  geom_errorbar(aes(ymin=Q_NH4-Q_NH4_sd, ymax=Q_NH4+Q_NH4_sd))+
...
Q_Act_Plot_SSA<- ggplot(ALL_Biochars,aes(x=NH4_Act,y=Q_NH4_SSA, color = OC,border= OC, fill=OC, na.rm=TRUE)) +
   geom_point(shape = 21, size = 6, stroke = 2, color = "black") +
  geom_errorbar(aes(ymin=Q_NH4_SSA-Q_NH4_SSA_sd, ymax=Q_NH4_SSA+Q_NH4_SSA_sd))+
  geom_errorbarh(aes(xmin=NH4_Act-NH4_Act_sd, xmax=NH4_Act+NH4_Act_sd))+
  # facet_wrap(~ Cation_Species, scales = "free")+ ###to separate into 3 plots
  xlab("Equilibrium Activity (mM)")+
...
Q_Act_Plot_OC<- ggplot(ALL_Biochars,aes(x=NH4_Act,y=Q_NH4_OC, color = OC,border= OC, fill=OC, na.rm=TRUE)) +
   geom_point(shape = 21, size = 6, stroke = 2, color = "black") +
  #geom_errorbar(aes(ymin=Q_NH4_SSA-Q_NH4_SSA_sd, ymax=Q_NH4_SSA+Q_NH4_SSA_sd))+
  #geom_errorbarh(aes(xmin=NH4_Act-NH4_Act_sd, xmax=NH4_Act+NH4_Act_sd))+
  # facet_wrap(~ Cation_Species, scales = "free")+ ###to separate into 3 plots
...
Q_Act_Plot_K<- ggplot(ALL_Biochars,aes(x=K_Act,y=Q_K, color = OC,border= OC, fill=OC, na.rm=TRUE)) +
   geom_point(shape = 21, size = 6, stroke = 2, color = "black") +
  # facet_wrap(~ Cation_Species, scales = "free")+ ###to separate into 3 plots
  xlab("Equilibrium Activity (mM)")+
...
Q_Act_Plot_Na<- ggplot(ALL_Biochars,aes(x=Na_Act,y=Q_Na, color = OC,border= OC, fill=OC, na.rm=TRUE)) +
   geom_point(shape = 21, size = 6, stroke = 2, color = "black") +
  # facet_wrap(~ Cation_Species, scales = "free")+ ###to separate into 3 plots
  xlab("Equilibrium Activity (mM)")+
  ylab("Q (mmol ion/g biochar)")+
...
ALL_Biochars_subset<-ALL_Biochars%>%
  filter(Q_NH4+Q_NH4_sd<6)
         
         #,Q_NH4-Q_NH4_sd>=-0.5)



Q_Conc_Plot_subset <- ggplot(ALL_Biochars_subset, aes(x = NH4_Act, y = Q_NH4, color = OC, fill = OC, na.rm = TRUE)) +
...
Norm_<-
  ALL_Biochars%>%
  select(c("OC","Q_NH4", "Q_Na", "Q_K","Q_Cl","Q_PO4","Q_SO4","Q_Ace", "Dose"))
...
Norm_long <-
  Norm_%>%
  pivot_longer(
    cols=c(Q_NH4, Q_Na, Q_K, Q_Cl, Q_PO4, Q_SO4, Q_Ace),
...
Q_Act_No_Err_Plot<- ggplot(ALL_Biochars,aes(x=NH4_Act,y=Q_NH4, color = OC,border= OC, fill=OC, na.rm=TRUE)) +
   geom_point(shape = 21, size = 6, stroke = 2, color = "black") +
  #geom_errorbar(aes(ymin=Q_NH4-Q_NH4_sd, ymax=Q_NH4+Q_NH4_sd))+
  #geom_errorbarh(aes(xmin=NH4_Act-NH4_Act_sd, xmax=NH4_Act+NH4_Act_sd))+
  # facet_wrap(~ Cation_Species, scales = "free")+ ###to separate into 3 plots

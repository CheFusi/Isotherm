...
Dilution_trip_2 <- Dilution_trip %>% 
   filter(!(Type_ox == "AK_30" & ID %in% c("16", "17", "18", "19", "20")) &
          !(Type_ox %in% c("AK_05") & ID %in% c("14", "15", "16", "17", "18", "19", "20")) &
          !(Type_ox %in% c("AK_10", "AM_15") & ID %in% c("14", "15", "16", "17", "18", "19", "20"))) #&
...
Dilution_for_PHREEQC <- Dilution_for_PHREEQC %>% select(-c("Mg"))
...
Activities for most diluted solutions.

```{r}

Dilution_trip_low <- Dilution_trip %>% 
   filter((Type_ox == "AK_30" & ID %in% c("11","12","13","14","15")) |
          (Type_ox %in% c("AK_05") & ID %in% c("11","12","13"))  |
          (Type_ox %in% c("AK_10", "AM_15") & ID %in% c("11","12","13")) |
          (Type_ox %in% c("AM_05") & ID %in% c("9","10")) |
          (Type_ox %in% c( "AM_10") & ID %in% c("9","10"))) 
...
Dilution_for_PHREEQC_low <- Dilution_for_PHREEQC_low %>% select(-c("Mg"))
...
Dilution_Act_low <- read.table("/Users/soliverchefusi/Dilution_Low_Activity.out", header = TRUE)
...
Separate back into a list of dfs

```{r}

# Split the dataframe into a list of dataframes based on the Type_ox column
df_act_list <- split(Dilution_trip_2, Dilution_trip_2$Type_ox)

# Name each dataframe in the list according to the unique values in the Type_ox column
#names(df_act_list) <- unique(Dilution_trip_2$Type_ox)
```
...
for (df_index in seq_along(df_act_list)) {
  # Get the name of the current dataframe
  df_name <- names(df_act_list)[df_index]
  
  # Create an empty dataframe to store the results
  averaged_df <- data.frame(ID = unique(df_act_list[[df_index]]$ID))  # Assuming Rep is present in the dataframe
  ...
  for (col_name in names(df_act_list[[df_index]])) {
    ...
    for (i in seq(1, nrow(df_act_list[[df_index]]), by = 3)) {
      # Extract the subset of rows for the current group
      group <- df_act_list[[df_index]][i:(i + 2), col_name]
...
### Add 0's to each df (for isotherm fitting, primarily)

```{r}

# Define a function to add the new row
add_new_row <- function(df) {
  # Create a new row with 0 for NH4_Act_avg and Q_NH4_avg, and NA for other columns
  new_row <- data.frame(matrix(NA, nrow = 1, ncol = ncol(df)))
  ...
  if ("ID" %in% colnames(df)) {
    max_id <- max(df$ID, na.rm = TRUE)
    new_row$ID <- max_id + 1
  } else {
    warning("ID column not found in the dataframe")
  }
  
  # Copy the Ox and Type_ox values from the first row
  if ("OX" %in% colnames(df)) {
    new_row$Ox <- df$Ox[1]
  } else {
    warning("Ox column not found in the dataframe")
  }
  
  if ("Type_ox" %in% colnames(df)) {
    new_row$Type_ox <- df$Type_ox[1]
  } else {
    warning("Type_ox column not found in the dataframe")
  }
  
  # Bind the new row to the end of the dataframe
...
Dilution_Isotherm_All <- lapply(Dilution_Isotherm_All, add_new_row)
...
generate_plot <- function(df, name) {
  ggplot(df, aes(x = NH4_Act_avg, y = Q_NH4_avg, na.rm = TRUE)) +
...
saveRDS(Dilution_Isotherm_All, "Dilution_Isotherms_All_Act.RDS")
```

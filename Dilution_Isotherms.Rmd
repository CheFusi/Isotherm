...
```{r}
# 
capacities_2 <- capacities %>% select("Q_NH4","Type_ox")
...
View(Ceq_dataframe %>% 
  select(contains("Cin")|contains("AM_15")|contains("AK_05")|contains("AK_10")|contains("AK_30")))
```
...
for (name in names(df_cat_in)) {
  ...
  # Create an index for grouping every three rows
  group_index <- df$ID
  
  # Calculate row-wise averages for every three rows (excluding "Rep" column)
  #avg_values <- tapply(rowMeans(df[, !colnames(df) %in% c("OX", "ID", "Rep")], na.rm = TRUE), group_index, mean)
  # Calculate row-wise averages for every three rows (excluding "Rep", "ID", and "OX" columns)
avg_values <- aggregate(. ~ ID, df[, !colnames(df) %in% c("OX", "ID", "Rep")], mean, na.rm = TRUE)
  print(avg_values)
  
  # Repeat the averages within each group of three rows to match the number of rows in the dataframe
  #repeated_avg_values <- rep(avg_values, each = 3)
  # Repeat the averaged values within each group of three rows to match the number of rows in the dataframe
repeated_avg_values <- rep(avg_values[match(group_index, avg_values$ID), -1], each = 3)

  print(repeated_avg_values)
  ...
  for (col_name in names(df)) {
    if (!(col_name %in% c("Rep", "OX", "ID"))) {
      ...
      # Create a new column with repeated averaged values
      df[[new_col_name]] <- repeated_avg_values
    }
  }
...
avg_values <- tapply(rowMeans(df_cat_in[["AK_30_Cat"]][, !colnames(df_cat_in[["AK_30_Cat"]]) %in% c("OX", "ID", "Rep")], na.rm = TRUE), group_index, mean)
```
...
for (prefix in unique_prefixes) {
  ...
  combined_df <- cbind(df_an_in_name, df_an_eq_name[,-c(1:3)], df_an_ctrl_name[,-c(1:3)], df_cat_in_name[,-c(1:3)], df_cat_eq_name[,-c(1:3)], df_cat_ctrl_name[,-c(1:3)], mass_biochar, volume)
  
  # Rename columns "OX_in", "ID_in", and "Rep_in"
  colnames(combined_df)[colnames(combined_df) %in% c("OX_in","ID_in","Rep_in")] <- c("OX","ID","Rep")
...
averaged_df <- data.frame(Rep = unique(combined_dfs[[1]]$ID))  # Assuming Rep_in is present in the first dataframe

# Iterate over each column in the combined dataframe
for (col_name in names(combined_dfs[[1]])) {
  # Skip the Rep_in column
  if (col_name %in% c("Rep")) {
    next
  }
...
saveRDS(averaged_dfs, "Dilution_Isotherms_All.RDS")

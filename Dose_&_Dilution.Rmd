...
unique_biochars <- unique(ALL_Biochars[, c("Type_ox", "OC", "SSA", "Type", "XPS_OC","OS_C","OD_C")])
...
Dilution_All$OS_C <- merged_df$OS_C[Dilution_All$match_idx]
Dilution_All$OD_C <- merged_df$OD_C[Dilution_All$match_idx]
...
### Dose Q-Plot Freundlich Q \< 3, Qsd \<25% No AL - C-O

```{r, warning=FALSE}

# Create the plot with unique data
Qmax_Plot <- ggplot(ALL_Biochars_Qsd_25_Q_3_Freund_No_AL, aes(x = OS_C, y = Q_400, color = Type, fill = Type)) +
  geom_point(shape = 21, size = 6, stroke = 2, color = "black") +
  geom_smooth(method = "lm", se = TRUE, color = "blue", aes(group = 1)) +  # Fit a single linear regression line for all data points
  xlab("OC Surface") +
  ylab("Q (mmol NH4/g biochar)") +
...
# Fit the linear regression model to the unique data
lm_model <- lm(Q_400 ~ OS_C, data = ALL_Biochars_Qsd_25_Q_3_Freund_No_AL)
...
# Add R-squared value to the plot
Qmax_Plot <- Qmax_Plot +
  geom_text(aes(x = max(OS_C), y = max(Q_400), label = paste("R^2 =", round(r_squared, 2))),
            hjust = 1, vjust = 5, color = "black", size = 6)
...
# Create the plot with unique data
Qmax_Plot <- ggplot(ALL_Biochars_Qsd_25_Q_3_Freund_No_AL, aes(x = OD_C, y = Q_400, color = Type, fill = Type)) +
  geom_point(shape = 21, size = 6, stroke = 2, color = "black") +
  geom_smooth(method = "lm", se = TRUE, color = "blue", aes(group = 1)) +  # Fit a single linear regression line for all data points
  xlab("C Double Bond O to C") +
  ylab("Q (mmol NH4/g biochar)") +
  theme_classic(base_size = 28) +
  theme(
    text = element_text(family = "Times"),
...
# Fit the linear regression model to the unique data
lm_model <- lm(Q_400 ~ OD_C, data = ALL_Biochars_Qsd_25_Q_3_Freund_No_AL)
...
# Add R-squared value to the plot
Qmax_Plot <- Qmax_Plot +
  geom_text(aes(x = max(OD_C), y = max(Q_400), label = paste("R^2 =", round(r_squared, 2))),
            hjust = 1, vjust = 5, color = "black", size = 6)
...
ALL_Biochars_Qsd_25_Q_3_Freund_No_AL_No_AO <- ALL_Biochars_Qsd_25_Q_3_Freund %>% 
  filter(!Type %in% c("AL","AO") & !is.na(XPS_OC) & !is.na(Q_400))
```

```{r, warning=FALSE}

Qmax_Plot <- ggplot(ALL_Biochars_Qsd_25_Q_3_Freund_No_AL_No_AO, aes(x = net_app_H_diff_avg_abs, y = Q_400, color = Type, fill = Type)) +
  geom_point(shape = 21, size = 6, stroke = 2, color = "black") +
  geom_smooth(method = "lm", se = TRUE, color = "blue", aes(group = 1)) +  # Fit a single linear regression line for all data points
  xlab("Net Apparent Proton Charge Difference") +
  ylab("Q (mmol NH4/g biochar)") +
...
# Fit the linear regression model to the unique data
lm_model <- lm(Q_400 ~ net_app_H_diff_avg_abs, data = ALL_Biochars_Qsd_25_Q_3_Freund_No_AL_No_AO)

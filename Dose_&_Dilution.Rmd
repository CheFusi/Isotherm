...
### Dose Q-Plot Freundlich Q \< 3, Qsd \<25%, high_diff
...
### Dose Q-Plot Freundlich Q \< 3, Qsd \<25%, high_diff No AL
...
### Dose Q-Plot Freundlich Q \< 3, Qsd \<25%, high_diff No AL - OC_XPS
...
### Dose Q-Plot Freundlich Q \< 3, Qsd \<25%, high_diff No AL Net App Diff
...
Qmax_Plot <- ggplot(ALL_Biochars_Qsd_25_Q_3_Freund_No_AL, aes(x = net_app_H_diff_avg_abs , y = Q_400, color = Type, fill = Type)) +
  geom_point(shape = 21, size = 6, stroke = 2, color = "black") +
  geom_smooth(method = "lm", se = FALSE, color = "blue", aes(group = 1)) +  # Fit a single linear regression line for all data points
  xlab("Net App Proton Charge Diff") +
  ylab("Q max (mmol NH4/g biochar)") +
  theme_classic(base_size = 28) +
  theme(
    text = element_text(family = "Times"),
...
Qmax_Plot <- Qmax_Plot +
  geom_text(aes(x = max(net_app_H_diff_avg_abs), y = max(Q_400), label = paste("R^2 =", round(r_squared, 2))),
            hjust = 1, vjust = 3, color = "black", size = 6)
...
Qmax_Plot <- ggplot(ALL_Biochars_Qsd_25_High_C_diff_nls, aes(x = OC, y = Q_400, color = Type, fill = Type)) +
  geom_point(shape = 21, size = 6, stroke = 2, color = "black") +
  geom_smooth(method = "lm", se = FALSE, color = "blue", aes(group = 1)) +  # Fit a single linear regression line for all data points
  xlab("OC Bulk") +
  ylab("Q max (mmol NH4/g biochar)") +
...
# Calculate R-squared value from the linear regression model
lm_model <- lm(Q_400 ~ OC, data = ALL_Biochars_Qsd_25_High_C_diff_nls)
r_squared <- summary(lm_model)$r.squared
